...
curl -s -X POST -H 'Content-Type: application/json' \
  -d '{
    ...
    "numbers": [
      ...
      {"number": "79990000102", "clientId": "pbx-zero", "TimeOffset": 0},
...
`TimeOffset: 5` означает UTC+5, `-4` — UTC−4. Значение `0`, пустая строка, `null` или отсутствие поля используют время АТС. Рабочий интервал применяется отдельно по местному времени каждого номера.
...
Ожидается: UTC+5 → `300`, время АТС с исходным нулём → `0`, UTC−4 → `-240`, время АТС с пустым значением → `null`. Хранимые `0` и `null` эквивалентны при проверке рабочего окна.
...
В объекте номера ненулевой `TimeOffset` задаёт смещение от UTC в часах (`5`, `-4`, `5.5`). Допустимый диапазон — от `-12` до `14`; значение должно соответствовать целому числу минут. Значение `0`, пустая строка, `null` или отсутствие поля означают использование локального времени АТС.
