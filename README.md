...
| `TimeOffset` | number/string | Смещение местного времени получателя от UTC в часах. Например, `5` — UTC+5, `-4` — UTC−4, `0` — UTC. Пустая строка, `null` или отсутствие поля означают использование времени АТС |
...
- при указанном `TimeOffset` используется местное время получателя относительно UTC;
- `5` означает UTC+5, `-4` — UTC−4, `0` — именно UTC;
- пустая строка, `null` или отсутствие `TimeOffset` означают локальное время АТС;
...
В GET-ответе нормализованное смещение находится в `data.results[].timeOffsetMinutes`: соответственно `300`, `0`, `-240` или `null`. Дополнительные сценарии и команды очистки приведены в [tests/curl-examples.md](tests/curl-examples.md).
...
| `timeOffsetMinutes` | integer \| null | Смещение получателя от UTC в минутах; `null` — использовать локальное время АТС |
