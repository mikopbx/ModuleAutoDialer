...
| `TimeOffset` | number/string | Ненулевое смещение местного времени получателя от UTC в часах. Например, `5` — UTC+5, `-4` — UTC−4. Значение `0`, пустая строка, `null` или отсутствие поля означают использование времени АТС |
...
При ненулевом `TimeOffset` поля задачи `timeStart` и `timeEnd` проверяются по местному времени конкретного получателя. При `TimeOffset: 0`, пустом значении или отсутствии поля они проверяются по времени АТС.

#### Ограничение звонков по времени

`timeStart` и `timeEnd` задаются на уровне задачи в минутах от начала суток, но проверяются отдельно для каждого номера:

- при ненулевом `TimeOffset` используется местное время получателя относительно UTC;
- `5` означает UTC+5, `-4` — UTC−4;
- `0`, пустая строка, `null` или отсутствие `TimeOffset` означают локальное время АТС;
- допустимы смещения от UTC−12 до UTC+14 и дробные часы, соответствующие целому числу минут, например `5.5` для UTC+5:30;
- границы включены: `480..1320` разрешает звонки с 08:00 до 22:00 включительно;
- если `timeStart > timeEnd`, окно проходит через полночь: `1320..360` означает 22:00–06:00;
- `0..1440` разрешает звонки круглосуточно.
...
> **Внимание:** задача с `state: 0` активна и может начать реальные звонки сразу после POST.
...
curl -sS -X POST -H 'Content-Type: application/json' \
  -d '{
    ...
    "numbers": [
      ...
      {"number": "79990000102", "clientId": "client-pbx-zero", "TimeOffset": 0},
...
В GET-ответе нормализованное смещение находится в `data.results[].timeOffsetMinutes`: соответственно `300`, `0`, `-240` или `null`. Хранимые `0` и `null` различаются, но при проверке окна оба используют время АТС. Дополнительные сценарии и команды очистки приведены в [tests/curl-examples.md](tests/curl-examples.md).
...
| `timeOffsetMinutes` | integer \| null | Смещение получателя от UTC в минутах; `0` и `null` используют локальное время АТС |
