...
Процедура СонхронизацияЗадач() Экспорт
	changeTime = Число(Формат(НачалоДня(ТекущаяДата()) - дата(1970,1,1,1,0,0), "ЧГ=0"));    		
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
    	changeTime = Число(ВыборкаДетальныеЗаписи.changeTime);
		Прервать;
	КонецЦикла;
	...
	Запрос.Текст = 
		...
		|	И ЗадачаНаОбзвонНомераТелефонов.Статус = 0
		|
		|СГРУППИРОВАТЬ ПО
...
// Переопределяемый метод формирования задач на обзвон	 
//
Процедура ФормированиеЗадачОбзвона() ЭкспортКонецПроцедуры